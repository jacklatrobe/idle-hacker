...
class IdleGame {
    ...
    gameLoop() {
        this.current_game_date = new Date(Date.now());
        this.current_cash = 0;
        this.current_job = new Job("Line Cook", "McDonalds", "15");
        this.previous_jobs = [];

        /* handle date change */
        IdleGame.gameLog("Rendering game date", LoggingLevel.INFO);
    ...
    render_page() {
        ...
        var dateObj = new Date(this.current_game_date);
        let pretty_date = dateObj.toLocaleDateString("en-AU");
        ...
        for (let old_job in this.previous_jobs) {
            let jobObj = previous_jobs[old_job]
...
class Education extends Advancable {
    ...
    toString() {
        let date_string = this.start_date.toLocaleString("en-AU", { year: "long" });
        if (this.end_date !== null && this.end_date !== undefined) {
            date_string = date_string + " - " + this.end_date.toLocaleString("en-AU", { year: "long" });
        }
        return (`${this.title} (${date_string})`);
    };
    render_dates() {
        let start_month = this.start_date.toLocaleString("en-AU", {
            month: "long",
        });
        let start_year = this.start_date.toLocaleString("en-AU", {
            year: "full",
        });
        let end_month = this.start_date.toLocaleString("en-AU", {
            month: "long",
        });
        let end_year = this.start_date.toLocaleString("default", { year: "full" });
        return start_month + " " + start_year + " - " + end_month + " " + end_year;
